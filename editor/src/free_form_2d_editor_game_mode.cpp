...
constexpr float kDefaultMoveSpeed = 220.f;
...
void FreeForm2DEditorGameMode::Start(EditorApp &app, const ProjectContext *project) {
  (void)project;
  ...
  try {
    ...
    std::fprintf(stderr, "[FreeFormPlay] snapshot failed: %s\n", e.what());
    worldSnapshot.clear();
  }
  ...
  if (!world.HasComponent<Controller>(playEntity)) {
    world.AddComponent<Controller>(playEntity, Vec2{kDefaultMoveSpeed, kDefaultMoveSpeed});
  ...
  std::printf("[FreeFormPlay] ▶ Started (entity %u). WASD / arrows — Shift run.\n",
              static_cast<unsigned>(playEntity));
}

void FreeForm2DEditorGameMode::Stop(EditorApp &app, bool revert) {
  World &world = app.GetWorld();
  ...
  if (revert && !worldSnapshot.empty()) {
    try {
      ...
      std::printf("[FreeFormPlay] World reverted to pre-play snapshot.\n");
    } catch (const std::exception &e) {
      std::fprintf(stderr, "[FreeFormPlay] ERROR reverting snapshot: %s\n", e.what());
    }
  ...
  *world.GetActiveCamera() = editorCamera;
  ...
  std::printf("[FreeFormPlay] ■ Stopped (revert=%s).\n", revert ? "true" : "false");
}

void FreeForm2DEditorGameMode::Tick(float dt) {
  (void)dt;
...
void FreeForm2DEditorGameMode::DrawHUD() {
  ...
  ImGui::SetNextWindowPos(ImVec2(vp->WorkPos.x + 10.f, vp->WorkPos.y + vp->WorkSize.y - 72.f));
  ImGui::SetNextWindowSize(ImVec2(360.f, 62.f));
  ImGui::SetNextWindowBgAlpha(0.55f);

  if (ImGui::Begin("##freeform_play_hud", nullptr, flags & ~ImGuiWindowFlags_NoBackground)) {
    ImGui::TextColored(ImVec4(0.85f, 0.9f, 1.f, 1.f), "Free-form 2D play");
    ImGui::TextDisabled("WASD / arrows  ·  Shift run  ·  Stop to exit");
  }
  ImGui::End();
}
